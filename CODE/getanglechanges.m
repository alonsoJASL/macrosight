...
% GET ANGLE CHANGES. From the track informaton table, a specific clump
% (wuc) and the range (initial to final frame).
%
% INPUT: 
%       TrInfo : 
%          wuc : 
%   clumprange :  
% controlrange : (optional) range of single motion for comparison
%
% OUTPUT:
%       contacttrackf  := clump track position and angle change
%       contactpos     := clump track features
%       benchmrktrackf := control track positions (optional)
%       benchmrkpos    := control track features  (optional)
%

if nargin < 4
    calculatebenchmarks = false;
elseif isempty(controlrange) || nargout < 4
    calculatebenchmarks = false;
else 
